...
function App() {
  ...
  async function saveAbsence(ab) {
    const record = { ...ab, id: ab.id || "ab" + Date.now() };
    const exists = absences.some((a) => a.id === record.id);
    const newAbs = exists ? absences.map((a) => a.id === record.id ? record : a) : [...absences, record];
    setAbsences(newAbs);
