...
function getGradeSubjectNames(gradeId) {
  if (gradeId === "e1" || gradeId === "e2") return ["\u56FD\u8A9E", "\u7B97\u6570", "\u751F\u6D3B", "\u97F3\u697D", "\u56F3\u5DE5", "\u4F53\u80B2", "\u9053\u5FB3", "\u5B66\u6D3B"];
  if (gradeId.startsWith("e")) return ["\u56FD\u8A9E", "\u7B97\u6570", "\u793E\u4F1A", "\u7406\u79D1", "\u97F3\u697D", "\u56F3\u5DE5", "\u4F53\u80B2", "\u5BB6\u5EAD", "\u5916\u56FD\u8A9E", "\u9053\u5FB3", "\u7DCF\u5408", "\u5B66\u6D3B"];
  return ["\u56FD\u8A9E", "\u6570\u5B66", "\u82F1\u8A9E", "\u793E\u4F1A", "\u7406\u79D1", "\u97F3\u697D", "\u7F8E\u8853", "\u6280\u8853", "\u5BB6\u5EAD", "\u4F53\u80B2", "\u9053\u5FB3", "\u7DCF\u5408", "\u5B66\u6D3B"];
}
var WEEKLY_ALLOC = {
...
async function loadJsPDF() {
  if (window._jsPDFLoaded) return { jsPDF: window.jspdf.jsPDF };
...
async function exportWeekPDF(weekData, teachers) {
  ...
  doc.text(`\u7B2C${weekData.weekNum}\u9031 \u6642\u9593\u5272`, 148, 80, { align: "center" });
  doc.setFontSize(13);
  doc.setTextColor(130, 130, 130);
  doc.text(weekData.label || `\u7B2C${weekData.weekNum}\u9031`, 148, 94, { align: "center" });
  if (weekData.note) {
    doc.setFontSize(10);
    doc.setTextColor(170, 170, 170);
    doc.text(weekData.note, 148, 108, { align: "center" });
  }
  GRADES.forEach((grade) => {
    doc.addPage();
    const tt = timetables[grade.id];
    ...
    doc.text(`${grade.label} \u6642\u9593\u5272 \u2014 \u7B2C${weekData.weekNum}\u9031`, 148, 13, { align: "center" });
    const head = [["", ...DAYS.map((d) => `${d}\u66DC\u65E5`)]];
    const body = Array.from({ length: grade.periods }, (_, p) => [`${p + 1}`, ...Array.from({ length: grade.days }, (_2, d) => {
      const cell = tt[d]?.[p];
      if (!cell) return "";
      if (cell.isEvent) return cell.eventLabel || "\u884C\u4E8B";
...
(${t.name})` : "");
    ...
    doc.autoTable({
      ...
      startY: 20,
      theme: "grid",
      styles: { font: "helvetica", fontSize: 10, halign: "center", valign: "middle", cellPadding: 3, minCellHeight: 12 },
      headStyles: { fillColor: grade.type === "j" ? [100, 90, 180] : [255, 100, 100], textColor: 255, fontSize: 9 },
      columnStyles: { 0: { fillColor: [245, 245, 245], fontStyle: "bold", cellWidth: 14 } },
      didParseCell(data) {
        if (data.section === "body" && data.column.index > 0) {
          ...
          const [r, g, b] = si.hex;
          data.cell.styles.fillColor = [r + Math.round((255 - r) * 0.6), g + Math.round((255 - g) * 0.6), b + Math.round((255 - b) * 0.6)];
          data.cell.styles.textColor = [Math.max(0, r - 60), Math.max(0, g - 60), Math.max(0, b - 60)];
  ...
  doc.save(`\u7B2C${weekData.weekNum}\u9031_\u6642\u9593\u5272.pdf`);
}
function App() {
  ...
  function handleCellTap(day, period) {
    ...
    if (!selected) {
      ...
      const tmp = grid[selected.day][selected.period];
      grid[selected.day][selected.period] = grid[day][period];
      grid[day][period] = tmp;
  ...
  function handleBaseCellTap(gradeId, day, period) {
    ...
    const cellName = baseTimetables[gradeId]?.[day]?.[period];
    if (cellName === "" || cellName === void 0) return;
    if (!baseSelected) {
      setBaseSelected({ gradeId, day, period });
    } else {
      if (baseSelected.gradeId === gradeId && baseSelected.day === day && baseSelected.period === period) {
        setBaseSelected(null);
        return;
      }
      if (baseSelected.gradeId !== gradeId) {
        setBaseSelected({ gradeId, day, period });
        return;
      }
      const base = { ...baseTimetables };
      const grid = base[gradeId].map((d) => [...d]);
      const tmp = grid[baseSelected.day][baseSelected.period];
      grid[baseSelected.day][baseSelected.period] = grid[day][period];
      grid[day][period] = tmp;
      base[gradeId] = grid;
      setBaseTimetables(base);
      setAllTimetables(applyEventsToBase(base, teachers, events, currentWeekNum, periodSettings));
  ...
  return /* @__PURE__ */ jsxs("div", { style: S.root, children: [
    ...
    /* @__PURE__ */ jsxs("div", { style: S.container, children: [
      ...
      conflictCount > 0 && /* @__PURE__ */ jsxs("div", { style: S.conflictBar, children: [
        "\u26A0\uFE0F \u5148\u751F\u306E\u30B3\u30DE\u304C\u91CD\u8907\u3057\u3066\u3044\u307E\u3059\uFF08",
        conflictCount,
        "\u4EF6\uFF09\u2014 \u300C\u5148\u751F\u7BA1\u7406\u300D\u3067\u4FEE\u6B63\u3059\u308B\u304B\u4F5C\u308A\u306A\u304A\u3057\u3066\u304F\u3060\u3055\u3044\u3002"
      ] }),
      view === "base" && /* @__PURE__ */ jsxs("div", { children: [
        ...
        !baseTimetables ? (
          ...
          /* @__PURE__ */ jsx("p", { style: { fontSize: "12px", color: "#AAA", marginBottom: "12px" }, children: "\u30B3\u30DE\u3092\u30BF\u30C3\u30D7\u3057\u3066\u9078\u629E \u2192 \u3082\u30461\u3064\u30BF\u30C3\u30D7\u3067\u5165\u308C\u66FF\u3048\uFF08\u5B66\u5E74\u3054\u3068\uFF09" }),
          ...
          baseSelected && baseSelected.gradeId === baseGrade.id && /* @__PURE__ */ jsxs("div", { style: S.swapHint, children: [
            "\u2705 ",
            /* @__PURE__ */ jsx("b", { children: baseTimetables[baseGrade.id]?.[baseSelected.day]?.[baseSelected.period] }),
            "\uFF08",
            baseSelected.period + 1,
            "\u6642\u9650\u76EE\u30FB",
            DAYS[baseSelected.day],
            "\u66DC\uFF09\u3092\u9078\u629E\u4E2D \u2014 \u5165\u308C\u66FF\u3048\u308B\u30B3\u30DE\u3092\u30BF\u30C3\u30D7",
            /* @__PURE__ */ jsx("button", { onClick: () => setBaseSelected(null), style: S.swapCancelBtn, children: "\u30AD\u30E3\u30F3\u30BB\u30EB" })
          ] }),
          /* @__PURE__ */ jsx("div", { style: { ...S.card, padding: "18px", overflowX: "auto" }, children: /* @__PURE__ */ jsx(
            BaseTimetableGrid,
            {
              ...
              teachers,
              teacherMap,
              selected: baseSelected?.gradeId === baseGrade.id ? baseSelected : null,
      ...
      view === "week" && /* @__PURE__ */ jsxs("div", { children: [
        ...
        /* @__PURE__ */ jsxs("div", { style: S.btnRow, children: [
          ...
          /* @__PURE__ */ jsx("button", { onClick: async () => {
            setPdfLoading(true);
            try {
              await exportWeekPDF({ weekNum: currentWeekNum, label: weekLabel || `\u7B2C${currentWeekNum}\u9031`, note: weekNote, timetables: serializeTimetables(allTimetables) }, teachers);
            } catch (e) {
              alert(e.message);
            } finally {
              setPdfLoading(false);
            }
          }, style: S.pdfBtn, disabled: pdfLoading, children: pdfLoading ? "\u23F3..." : "\u{1F4C4} PDF" }),
          saveMsg && /* @__PURE__ */ jsx("span", { style: S.saveMsg, children: saveMsg })
        ] }),
      ...
      view === "single" && /* @__PURE__ */ jsxs(Fragment, { children: [
        ...
        /* @__PURE__ */ jsxs("div", { style: S.btnRow, children: [
          ...
          saveMsg && /* @__PURE__ */ jsx("span", { style: S.saveMsg, children: saveMsg }),
          /* @__PURE__ */ jsx("span", { style: S.hint, children: "\u30B3\u30DE\u3092\u30BF\u30C3\u30D7\u3057\u3066\u9078\u629E \u2192 \u3082\u30461\u3064\u30BF\u30C3\u30D7\u3067\u5165\u308C\u66FF\u3048" })
        ] }),
        ...
        /* @__PURE__ */ jsxs("div", { style: { ...S.card, padding: "18px", opacity: animating ? 0 : 1, transition: "opacity .3s", overflowX: "auto" }, children: [
          selected && /* @__PURE__ */ jsxs("div", { style: S.swapHint, children: [
            "\u2705 ",
            /* @__PURE__ */ jsx("b", { children: allTimetables[selectedGrade.id]?.[selected.day]?.[selected.period]?.name }),
            "\uFF08",
            selected.period + 1,
            "\u6642\u9650\u76EE\u30FB",
            DAYS[selected.day],
            "\u66DC\uFF09\u3092\u9078\u629E\u4E2D \u2014 \u5165\u308C\u66FF\u3048\u308B\u30B3\u30DE\u3092\u30BF\u30C3\u30D7 ",
            /* @__PURE__ */ jsx("button", { onClick: () => setSelected(null), style: S.swapCancelBtn, children: "\u30AD\u30E3\u30F3\u30BB\u30EB" })
          ] }),
          /* @__PURE__ */ jsx(
            TimetableGrid,
            {
      ...
      view === "all" && /* @__PURE__ */ jsxs(Fragment, { children: [
        /* @__PURE__ */ jsxs("div", { style: S.btnRow, children: [
          ...
          /* @__PURE__ */ jsx("button", { onClick: async () => {
            setPdfLoading(true);
            try {
              await exportWeekPDF({ weekNum: currentWeekNum, label: weekLabel, note: weekNote, timetables: serializeTimetables(allTimetables) }, teachers);
            } catch (e) {
              alert(e.message);
            } finally {
              setPdfLoading(false);
            }
          }, style: S.pdfBtn, disabled: pdfLoading, children: pdfLoading ? "\u23F3..." : "\u{1F4C4} \u5168\u5B66\u5E74PDF" }),
          saveMsg && /* @__PURE__ */ jsx("span", { style: S.saveMsg, children: saveMsg })
        ] }),
            ...
            /* @__PURE__ */ jsxs("div", { style: S.historyCardTop, children: [
              ...
              /* @__PURE__ */ jsxs("div", { style: S.historyCardActions, children: [
                ...
                /* @__PURE__ */ jsx("button", { onClick: async () => {
                  setPdfLoading(true);
                  try {
                    await exportWeekPDF(week, week.teachers || teachers);
                  } catch (e) {
                    alert(e.message);
                  } finally {
                    setPdfLoading(false);
                  }
                }, style: S.historyPdfBtn, disabled: pdfLoading, children: pdfLoading ? "\u23F3" : "\u{1F4C4}" }),
...
function TimetableGrid({ grade, timetable, teachers, teacherMap, conflicts, highlightTeacher, selected, onCellTap }) {
  return /* @__PURE__ */ jsxs("div", { style: { display: "grid", gap: "5px", gridTemplateColumns: `48px repeat(${grade.days},1fr)`, minWidth: "320px" }, children: [
    ...
    Array.from({ length: grade.periods }, (_, p) => /* @__PURE__ */ jsxs(React.Fragment, { children: [
      ...
      Array.from({ length: grade.days }, (_2, d) => {
        ...
        if (!cell) return /* @__PURE__ */ jsx("div", {}, `${d}-${p}`);
        if (cell.isEmpty) return /* @__PURE__ */ jsx("div", { style: { ...S.cell, background: "#F8F9FA", borderColor: "#EEEEEE", borderStyle: "dashed", cursor: "default", justifyContent: "center", opacity: 0.4 }, children: /* @__PURE__ */ jsx("span", { style: { fontSize: "18px", color: "#CCC" }, children: "\u2014" }) }, `${d}-${p}`);
        if (cell.isEvent) return /* @__PURE__ */ jsxs("div", { style: { ...S.cell, background: cell.color + "22", borderColor: cell.color + "66", borderStyle: "dashed", cursor: "default", justifyContent: "center" }, children: [
          /* @__PURE__ */ jsx("span", { style: { fontSize: "18px" }, children: cell.emoji }),
        ...
        const isTarget = selected && !(selected.day === d && selected.period === p);
        const isHl = highlightTeacher && tid === highlightTeacher;
        const isConflict = conflicts.some((c) => c.teacherId === tid && c.day === d && c.period === p);
        return /* @__PURE__ */ jsxs(
          "div",
          {
            ...
            style: {
              ...
              borderColor: isConflict ? "#FF4757" : isSelected ? cell.color : isTarget ? "#667eea" : cell.color + "88",
              borderWidth: isSelected || isConflict || isTarget ? 2 : 1,
              borderStyle: isTarget ? "dashed" : "solid",
              transform: isSelected ? "scale(0.96)" : "scale(1)",
              boxShadow: isConflict ? "0 0 0 2px #FF4757" : isSelected ? `0 0 0 3px ${cell.color}` : isHl ? `0 0 0 3px ${teacher?.color}` : "none",
